...
def show_ai_qna(df, api_key):
    ...
    question = st.text_area("Ask a question about the data (e.g., 'Which channel should I prioritize?', 'Why did revenue drop for X?')", height=70)
    
    if st.button("Ask AI"):
        with st.spinner("AI is thinking..."):
            try:
                # Convert the first 500 rows to CSV for AI context (avoid token limit)
